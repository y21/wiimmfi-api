    ...
    get('https://wiimmfi.de/mkw/list', result => {
        ...
        result.on('end', () => {
            ...
            res.json({
                status: 200,
                data: {
                    VR: ((userMatch[0] || []).filter(e=>/\d{4}/g.test(e)) || []).sort((a,b)=>str.indexOf(a)-str.indexOf(b))[0],
                    BR: ((userMatch[0] || []).filter(e=>/\d{4}/g.test(e)) || []).sort((a,b)=>str.indexOf(a)-str.indexOf(b))[1],
